...
<mat-card>
      ...
      <form [formGroup]="formTotal" class="grid grid-cols-1 md:grid-cols-3 gap-4" style="text-align: center;">
...
export class TotaisResumoComponent implements OnChanges{
  @Input() produtos: any[] = [];
  ...
  // calcula a soma dos imput
 ngOnChanges(changes: SimpleChanges): void {
  if (this.formTotal && this.produtos.length > 0) {
    this.formTotal.patchValue({
      baseCalculo: this.totalBruto,
      vrIcms: this.totalIcms,
      vrTotalProd: this.totalBruto,
      vrTotalNfe: this.totalLiquido
    });
  }
}


  // Calcula o valor total de ICMS da nota somando o ICMS de cada item
